...
## Task 9: PR review round — GitHub bot comments, exception-pairing bugs, and the `helix.pipelines` field gap

**Context:** After Task 8 shipped, a code-review pass over the PR's actual diff (triggered by GitHub bot comments plus a request to hunt for bugs and check cross-repo compatibility) found four things worth recording:

1. **Two `aikido-pr-checks[bot]` comments** on `simulated_graph_processor_mixin.py` flagged the `on_resource_type_completed` handling as nested 6-7 levels deep (`try`/`while`/`for`/`async for`/conditional). Fixed by extracting the firing + declared-type-fallback logic into a new private method, `_fire_on_resource_type_completed_for_link`. Also dropped a redundant `if link_queried_urls:` guard around `all_urls.update(...)` — updating a set with an empty list is already a no-op. Two older `claude[bot]` comments (conventional-commit examples, the `graph_depth` docstring/implementation mismatch) were confirmed already resolved by this plan's own earlier revisions — stale, no action needed.
2. **Real bug: exceptions broke the started/completed pairing invariant Task 8 built.** If the start resource's fetch raised, or a link's fetch raised inside `process_link_async_parallel_function`, the matching `on_resource_type_completed` never fired — only the whole-graph `on_graph_retrieval_completed` bookend was actually guaranteed (Task 8's own scope). Fixed by wrapping both fetch points in `try`/`except`, firing `on_resource_type_completed` (resource_count=0, declared-type fallback, urls=[]) before re-raising the original exception unchanged — this is purely an additional signal, never error suppression. Required adding `on_resource_type_completed` to `GraphLinkParameters` (defaulted `None`, so `simulate_graph_async()`'s unrelated call site needs no change).
3. **Real bug: `on_resource_type_started` and the completion fallback were asymmetrically gated.** `on_resource_type_started` only fired when `row.target` was non-empty; the completion fallback fired unconditionally. A link with no declared `target` (legal — `GraphDefinitionLink.from_dict` defaults it to `[]`) got a `completed` event with no matching `started`. Fixed by making `on_resource_type_started` fire unconditionally too, with `resource_types=[]` when nothing is declared — restoring symmetry.
4. **Cross-repo compatibility gap:** reviewing `helix.pipelines`' updated companion plan (`~/git/helix.pipelines/docs/superpowers/plans/2026-08-22-proa-per-resource-type-progress.md` and its design doc) found it depends on `client_person_id`/`connection_name` fields on all four SDK events — fields this plan never specified (Tasks 4-8 only ever added `urls`/`url` for correlation). `helix.pipelines`' plan is explicit and detailed about this (exact call-site code passing `client_person_id=connection.client_fhir_person_id or ""`, `connection_name=connection.display_name or "not provided"`, and reading `event.client_person_id`/`event.connection_name` back off every fired event). Added both as caller-supplied, opaque keyword-only parameters on `simulate_graph_by_resource_type_async()` (default `""`), echoed back unmodified on all four event dataclasses, threaded through every construction site including the new exception-handling paths from fix #2 above. `service_slug` (also in `helix.pipelines`' field tables) is explicitly captured via closure at that repo's own call site, per its own plan — not an SDK field, not added here.

**Files:** `helix_fhir_client_sdk/graph/simulated_graph_processor_mixin.py`, `helix_fhir_client_sdk/graph/graph_link_parameters.py`, all four event dataclass files, their test files, `test_simulate_graph_by_resource_type_async_completion_hook.py`. No changes to `~/git/helix.pipelines` — read-only reference for field-shape reconciliation, that repo's actual implementation is out of scope here.

Commits: `3374997` (nesting extraction), `23d3dbf` (exception-pairing + symmetry fixes, plus two new regression tests), `d4618b0` (client_person_id/connection_name).
