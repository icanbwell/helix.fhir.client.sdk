...
class SizeCalculator:
    ...
    def get_recursive_size(obj: Any, seen: Optional[Set[int]] = None) -> int:
        ...
        if hasattr(obj, "__dict__"):
            ...
            size += SizeCalculator.get_recursive_size(obj.__dict__, seen)
        ...
        if isinstance(obj, (tuple, list, set, frozenset)):
            size += sum(SizeCalculator.get_recursive_size(item, seen) for item in obj)

        # Handle dictionaries
        elif isinstance(obj, dict):
            size += sum(
                SizeCalculator.get_recursive_size(k, seen)
                + SizeCalculator.get_recursive_size(v, seen)
        ...
        # Handle custom objects
            ...
            for slot_name in obj.__class__.__slots__:
                if hasattr(obj, slot_name):
                    ...
                    size += SizeCalculator.get_recursive_size(slot_value, seen)
            ...
            for attr_name in dir(obj):
                ...
                if not attr_name.startswith("__") and not callable(
                    ...
                    try:
                        ...
                        size += SizeCalculator.get_recursive_size(attr_value, seen)
                    except Exception:
