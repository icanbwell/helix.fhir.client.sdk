...
Content-Type header (in particular application/fhir+ndjson when use_data_streaming()
is enabled) on every merge code path, since the FHIR server only parses the request
body as a streamed ndjson batch for $merge when it is told to via the Content-Type
header.
"""
...
@pytest.mark.asyncio
async def test_merge_async_sends_ndjson_content_type_header_when_streaming_enabled() -> None:
    ...
    with aioresponses() as m:
        ...
        assert _sent_headers(m, "POST", url)["Content-Type"] == "application/fhir+ndjson"


@pytest.mark.asyncio
async def test_merge_async_sends_default_content_type_header_when_streaming_disabled() -> None:
    url = "http://example.com/Patient/1/$merge"
    ...
    with aioresponses() as m:
        ...
        assert _sent_headers(m, "POST", url)["Content-Type"] == "application/fhir+json"


@pytest.mark.asyncio
async def test_merge_resources_async_sends_ndjson_content_type_header_when_streaming_enabled() -> None:
    url = "http://example.com/Patient/1/$merge"
    fhir_client = FhirClient().url("http://example.com").resource("Patient").use_data_streaming(True)
    json_data_list = [{"resourceType": "Patient", "id": "1"}]
...
@pytest.mark.asyncio
async def test_merge_bundle_uncompressed_sends_ndjson_content_type_header_when_streaming_enabled() -> None:
    url = "http://example.com/Patient/1/$merge"
    fhir_client = FhirClient().url("http://example.com").resource("Patient").use_data_streaming(True)
    bundle = FhirBundle(
        type_="batch",
...
@pytest.mark.asyncio
async def test_merge_bundle_async_sends_ndjson_content_type_header_when_streaming_enabled() -> None:
    url = "http://example.com/Patient/1/$merge"
    fhir_client = FhirClient().url("http://example.com").resource("Patient").use_data_streaming(True)
    bundle = FhirBundle(
        type_="batch",
