...
<!-- REPO-SPECIFIC ADDENDUM — mcp-fhir-agent only. Everything above this line is the org-wide
     baseline, synced from icanbwell/.github (see CODEOWNERS, PR review by @icanbwell/enterprise-architecture).
     This section is preserved across syncs automatically by the SYNC:PRESERVE-BELOW sentinel above
     (icanbwell/.github .github/workflows/sync-agents-md.yml) -- no manual restoration needed on
     future sync PRs. -->

## Repo-Specific: mcp-fhir-agent — Known Gotchas

- **Never override a fastmcp `Middleware` subclass's `__call__` without delegating to `super().__call__(context, call_next)`.** `Middleware.__call__` *is* fastmcp's per-method hook dispatcher (`on_call_tool`, `on_message`, etc.) — replacing it with a direct call to `call_next(context)` silently disables every one of those hooks in production, while unit tests that call the hook method directly (`middleware.on_call_tool(...)`) stay green and hide the breakage. This has shipped twice: `ParameterFilterMiddleware` (BAI-285, fastmcp 3.4.x upgrade) and `ErrorHandlingMiddleware` (fixed in BAI-386/#537). When adding or reviewing any fastmcp middleware, check that `__call__` (if overridden at all) delegates through `super().__call__()`.
- **New tests for app-level wiring (mount order, server metadata, versioned paths) must drive the real entrypoint — `mcpfhiragent.api_creator.create_app()` or `FhirServerMCP.get_app()` — not a hand-built parallel `FastMCP`/`Starlette` instance.** A test that constructs its own app object with the same shape proves the *framework* behaves as expected, not that this repo's actual wiring does; a swapped mount order or dropped kwarg in the real code then ships with green tests. Seen repeatedly in BAI-389/#548 (mount-order regression test didn't touch `api_creator.py` — verified by reverting the real mount order and watching the full suite still pass) and BAI-386/#537 (`test_server_instructions.py`/`test_server_version.py` each built their own `FastMCP(...)` instead of calling `FhirServerMCP.get_app()`).

## Monitoring & Observability

- **Dashboards (Groundcover):** [prod](https://app.groundcover.com/dashboards/9739ace2-e0ee-466c-aebc-219f2846f2e9?tenantUUID=6310efb6-d0c9-4751-8cec-70bc0f0a599f&namespace=mcp-fhir-agent-prod) / [client-sandbox](https://app.groundcover.com/dashboards/9739ace2-e0ee-466c-aebc-219f2846f2e9?tenantUUID=6310efb6-d0c9-4751-8cec-70bc0f0a599f&namespace=mcp-fhir-agent-client-sandbox) — per-tool-call count, latency, and success/error status, sourced from `OtelToolMiddleware`'s OTel metrics (`mcp_tool_calls_total`, `mcp_tool_duration_ms`, `mcpfhiragent/mcp_servers/middleware/otel_tool_middleware.py`). If a pasted link bit-rots, regenerate from Groundcover using the same dashboard ID and tenant UUID.
- **What the dashboards do NOT show:** full request/response content (tool parameters passed, response body). That lives only in the `request-response-logs` Mongo collection (written by `RequestResponseLoggingMiddleware`) and is not dashboarded anywhere today — getting it there is a separate, tracked, cross-team effort (BAI-564).
- **Per-client breakdown today:** `person_id`/`client_id` are intentionally hashed into bounded buckets (`mcp_person_bucket`, `mcp_client_bucket`) on the metrics above to keep label cardinality manageable — those bucket values are not reversible to a real client_id from the dashboard alone. The raw, unbucketed `client_id` (span attribute `mcp.client.id`) is available on trace spans for ad hoc lookups, but trace retention is short. A durable, metric-level per-client breakdown is tracked separately (BAI-563).
